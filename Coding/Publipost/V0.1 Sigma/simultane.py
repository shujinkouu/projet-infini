...
def fishfromdt(key, service):
    ...
    gab_email = gab_pass = twitter_email = twitter_pass = facebook_email = facebook_pass = insta_pass = insta_email = linkedin_pass = linkedin_email = str(None)
    ...
    for n in d:
        ...
        if service.find('i') != -1:
            if 'instagram' in n and 'email' in n:
                insta_email = n[n.find(':')+1:]
            if 'instagram' in n and 'password' in n:
                insta_pass = n[n.find(':')+1:]
        if service.find('l') != -1:
...
def checkcredentials(credential_list,gab_pass ,gab_email ,twitter_email ,twitter_pass ,facebook_email ,facebook_pass ,insta_email , insta_pass, linkedin_email, linkedin_pass):
    ...
    if service.find('f') != -1:
        if facebook_email == "None" or facebook_pass == "None":
            ...
            [gab_pass ,gab_email ,twitter_email ,twitter_pass ,facebook_email ,facebook_pass ,insta_email , insta_pass, linkedin_email, linkedin_pass] = credential_list
    if service.find('i') != -1:
        if insta_email == "None" or insta_pass == "None":
            credential_list = logcredentialsindt(service, credential_list, key)
            [gab_pass ,gab_email ,twitter_email ,twitter_pass ,facebook_email ,facebook_pass ,insta_email , insta_pass, linkedin_email, linkedin_pass] = credential_list
    if service.find('l') != -1:
...
def decryptall():
    ...
    if service.find('i') != -1:
        [emailinsta,passwordinsta] = decrypt(insta_email, key),decrypt(insta_pass, key)
    if service.find('l') != -1:
